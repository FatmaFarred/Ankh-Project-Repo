import 'package:ankh_project/domain/entities/all_marketers_entity.dart';
import 'package:ankh_project/domain/entities/all_products_entity.dart';
import 'package:dartz/dartz.dart';
import 'package:injectable/injectable.dart';

import '../../api_service/failure/error_handling.dart';
import '../repositries_and_data_sources/repositries/home_get_all_products_repositry.dart';
import '../repositries_and_data_sources/repositries/marketer_assign _get_products_repositry.dart';

@injectable
class MarketerSearchUseCase {
  MarketerAssignGetProductsRepositry repository;

  MarketerSearchUseCase(this.repository);

  Future<Either<Failure, List<AllMarketersEntity>>> execute(String keyWord) {
    return repository.searchMarketer(keyWord);
  }

}

